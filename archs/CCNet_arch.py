...
import numpy as np
from archs.color_utils.soft_encode_ab import SoftEncodeAB
from archs.color_utils.annealed_mean_decode_q import AnnealedMeanDecodeQ
from archs.color_utils.cielab import ABGamut, CIELAB, DEFAULT_CIELAB
from archs.color_utils.colorspace_transform import rgb2lab,lab2rgb
...
class BCNet(nn.Module):
    ...
    def forward(self, input,gt,ref = None,saturation = None):
        B, C, H, W = input.shape
        rate = 2 ** 4
        pad_h = (rate - H % rate) % rate
        pad_w = (rate - W % rate) % rate
        if ref is not None:
            c_ref, _ = self.get_chrominance(ref, ref, self.color_space)
        else:
            c_ref = None
        if pad_h != 0 or pad_w != 0:
            input = F.pad(input, (0, pad_w, 0, pad_h), "reflect")
            if c_ref is not None:
                c_ref = F.pad(c_ref, (0, pad_w, 0, pad_h), "reflect")
        ...
        if self.phase=='lightness':
            ...
            if self.color_space != 'Lab':
                ab_out, q_pred = self.chrominance(c_input, l_fea, c_ref,saturation)
                q_actual = None
